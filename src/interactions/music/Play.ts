...
export default class Play extends Interaction {
    ...
    async run(context: InteractionContext): Promise<void> {
        ...
        // const isURL = await urlExist(query);

        // if (isURL) {
        //     if (!isSpotify) {
        //         ytdl.getInfo(query).then(info => {
        //             query = (info.videoDetails.title)
        //             console.log(`Convert link to ${query}`)
        //         })
        //     } else return await context.sendInteractionMessage(
        //         "Spotify link isn't support, yet."
        //     );
        // }
        ...
        if (result.loadType === LoadType.PLAYLIST)
            await context.sendInteractionMessage(
                `Loaded ${result.data.info.name} with ${result.data.tracks.length} track(s) in queue!`
            );
        else
            await context.sendInteractionMessage(
                `Loaded ${track!.info.title} in the end of the queue!`
            );
    }
}
