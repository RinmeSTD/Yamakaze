...
export const CommandData = new SlashCommandBuilder()
    ...
    .addSubcommand((option) =>
        option.setName("off").setDescription("Turns off the repeat mode")
    )
    .addSubcommand((option) =>
        option.setName("once").setDescription("Repeat the currently playing track")
    )
    .addSubcommand((option) =>
        option.setName("all").setDescription("Repeat the whole track queue")
    )
    .toJSON();
...
export default class Repeat extends Interaction {
    ...
    constructor(client: Yamakaze, directory: string) {
        ...
        this.commandOptions = {
            music: { channel: true, author: true, queue: true },
        };
    }
    ...
    async run(context: InteractionContext): Promise<void> {
        ...
        await context.sendInteractionMessage(
            `Playback repeat is set to: ${command.toUpperCase()}`
        );
    }
}
