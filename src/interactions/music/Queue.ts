...
export const CommandData = new SlashCommandBuilder()
    ...
    .addNumberOption((option) =>
        option.setName("page").setDescription("Queue page to view")
    )
    .toJSON();
...
export default class Queue extends Interaction {
    ...
    static generateEntry(track: UserTrack, page: number, index: number) {
        return `\`${page + index}}\` \`${track.info.title} [${ReadableTime(
            track.info.length
        )}]\` - <@${track.userId}>`;
    }
    ...
    async run(context: InteractionContext): Promise<void> {
        ...
        if (!tracks.length) embed.setDescription("`No track(s) in queue...`");
        else
            embed
                .setDescription(
                    tracks
                        .map((track, index) => Queue.generateEntry(track, page, index))
                        .join("\n")
                )
