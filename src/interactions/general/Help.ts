...
export default class Help extends Interaction {
    ...
    async run(context: InteractionContext): Promise<void> {
        ...
        for (const command of this.client.interactions.commands.values()) {
            ...
            const subCommands =
                command.commandData.options?.filter(
                    (options) => options.type === ApplicationCommandOptionType.Subcommand
                ) || [];
            ...
            for (const subCommandGroup of subCommandGroups) {
                ...
                for (const subCommand of subCommandGroup.options)
                    category.push(
                        `/${command.commandData.name} ${subCommandGroup.name} ${subCommand.name}`
                    );
            }
        }
        ...
        const embed = new EmbedBuilder()
            .addFields(...fields)
            .setFooter({
                text: `${this.client.interactions.commands.size} command(s) available`,
            });
