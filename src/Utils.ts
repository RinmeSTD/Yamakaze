...
export const Config: ConfigOptions = JSON.parse(
  Fs.readFileSync("./Config.json", { encoding: "utf-8" })
);

export const Lavalink: NodeOption[] = JSON.parse(
  Fs.readFileSync("./Lavalink.json", { encoding: "utf-8" })
);
...
export const YamakazeAsciiArt = "welcome";
/*
...
export function Paginate<T>(array: T[], page: number, limit: number): T[] {
  ...
  return duplicate.length > limit
    ? duplicate.slice(start, start + limit)
    : duplicate;
}
...
export function Shuffle<T>(array: T[]): T[] {
  ...
  for (let length = duplicate.length - 1; length > 0; length--) {
    ...
    [duplicate[length], duplicate[random]] = [
      duplicate[random],
      duplicate[length],
    ];
  }
